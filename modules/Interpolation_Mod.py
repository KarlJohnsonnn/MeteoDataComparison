...
def Interpolate_2D(x1, y1, z1, x2, y2, method):
    ...
    if method in ['linear', 'quadratic', 'cubic', 'quintic']:
        ...
        print('  CPU Time create interpolation function = {:.2f}'.format(time.time()-t0))
        ...
        print('  CPU Time interpolation = {:.2f}'.format(time.time()-t0))
...
def Interpolate_2D_neu(ds1, z_inter, ds2, method):
    ...
    elif method in ['NearestNeighbour','biliniear']:


        coord = np.empty((len_x2 * len_y2, 2))
        cnt = 0
